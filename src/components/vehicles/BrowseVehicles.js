...
import {
  Car, Search, SlidersHorizontal,
  Users, Fuel, Settings2,
} from 'lucide-react'

export default function BrowseVehicles({ vehicles, categories }) {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map((v) => {
            ...
            return (
              <div
                key={v.id}
                onClick={() => router.push(`/dashboard/browse/${v.id}`)}
                className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden hover:border-blue-500/50 hover:shadow-lg hover:shadow-blue-500/5 transition-all group cursor-pointer"
              >
                {/* Image */}
